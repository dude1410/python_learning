...
def triangle(point, color, angle=0):

    for i in range(3):
        v = sd.get_vector(start_point=point, angle=angle, length=200, width=3)
        v.draw(color=color)
        point = v.end_point
        angle = angle + 120
...
for angle in range(0, 361, 30):
    triangle(point=point_0, angle=angle, color=sd.COLOR_DARK_YELLOW)
